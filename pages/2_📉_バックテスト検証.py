...
def run_ai_backtest(ticker, rule_text):
    ...
    prompt = f"以下のルールをデータに適用し、全トレードをJSONリスト [{{'side':'buy','entry_price':0,'exit_price':0,'entry_time':'...','exit_time':'...','reason':'...'}}] のみで出力せよ。\n\n【ルール】\n{rule_text}\n\n【データ】\n{price_data}"
    try:
...
if "saved_rule_text" in st.session_state and st.session_state.saved_rule_text:
    ...
    if st.button("🚀 精密バックテストを実行", type="primary", use_container_width=True):
        with st.spinner("AIが全トレードを厳格シミュレーション中..."):
            ...
            if trades:
                ...
                for t in trades:
                    ...
                    history.append({"結果": win_loss, "売買": t.get('side'), "損益": f"{profit_yen:+,}円", "エントリー": e, "決済": ex, "理由": t.get('reason')})
                
                st.write("### 📊 バックテスト結果まとめ")
                c1, c2, c3 = st.columns(3)
                c1.metric("最終資産", f"{int(balance):,} 円")
                c2.metric("合計損益", f"{int(balance - initial_balance):+,} 円")
