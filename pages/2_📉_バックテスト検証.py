...
def run_ai_backtest(ticker, rule_text):
    ...
    prompt = f"以下のルールをデータに適用し、全トレードをJSONリスト [{{'side':'buy','entry_price':0,'exit_price':0,'entry_time':'...','exit_time':'...','reason':'...'}}] で出力せよ。\n\n【ルール】\n{rule_text}\n\n【データ】\n{price_data}"
    try:
...
if "saved_rule_text" in st.session_state and st.session_state.saved_rule_text:
    ...
    if st.button("🚀 精密バックテストを実行", type="primary", use_container_width=True):
        with st.spinner("AIが全トレードを計算中..."):
            ...
            if trades:
                balance, initial_balance, win_count, history = 1000000, 1000000, 0, []
                for t in trades:
                    ...
                    history.append({"結果": win_loss, "損益(円)": f"{profit_yen:+,}円", "エントリー": e, "決済": ex, "時間": t.get('entry_time'), "残高": f"{int(balance):,}円", "理由": t.get('reason')})
                
                st.write("### 📊 バックテスト結果まとめ")
...
else: st.info("分析室ページでルールを作成してください。")
